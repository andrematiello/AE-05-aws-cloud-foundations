...
a manual handoff — this environment has no AWS credentials for a personal account — documented step
by step in [`docs/aws_deploy_handoff.md`](docs/aws_deploy_handoff.md).
...
S3 static hosting, an EC2 instance serving a Streamlit app reading a CSV from S3) — but instead of a
generic tutorial dataset, both deployments serve real output from this portfolio's own **AE-01**
project: its dbt documentation site and its dimensional-model market data.

## ❓ Business Problem

How do you put an Analytics Engineering deliverable — dbt's auto-generated documentation, and a
dbt mart — in front of someone who isn't going to clone a repo and run `dbt docs serve` locally?
Static hosting and a small serving layer, provisioned as code and secured with least-privilege
access, is the minimum real answer.
...
1. **Docs → S3 static website.** `dbt docs generate` produces a self-contained static site
   (`index.html` + JSON manifests). No server needed — S3 website hosting serves it directly.
2. **Marts → S3 (private) → EC2 → Streamlit.** The CSV is not public. The EC2 instance reads it
   through an IAM instance role scoped to `s3:GetObject`/`s3:ListBucket` on that one bucket —
   nothing else, no console access, no other service.

## 🗂️ Data

- **Source:** AE-01's `warehouse.duckdb` — read-only queries only, this project never writes to it.
- **`market_prices.csv`:** `fct_daily_prices` joined to `dim_tickers` — 5,010 rows, one per ticker
  per trading day (10 tickers, 2024-07-15 → 2026-07-14), with `sector`/`industry` attached for the
  dashboard's filter. Regenerated by `scripts/export_marts_to_csv.py`; committed to this repo so
  the app runs the same way locally and on EC2.
- **dbt docs site:** `manifest.json` + `catalog.json` + `index.html`, regenerated by
  `dbt docs generate` in AE-01 — the same lineage graph and column documentation described in
...
1. **Export (read-only):** `scripts/export_marts_to_csv.py` connects to AE-01's DuckDB warehouse in
   ...
   project.
2. **Infrastructure as code:** Terraform defines every resource — two S3 buckets (one public
   website, one private with default encryption and a public-access block), one EC2 instance behind
   a security group that only opens 22 (from a single trusted CIDR) and 80, one IAM role, and an
   AWS Budgets alert.
...
4. **App code, tested independently of AWS:** `streamlit_app/app.py` reads its data source from an
   ...
   `s3://` URI resolved through the instance role. The same code path, verified with a local run
   (see [Results](#-results)) and ready to point at S3.

## 🤔 Design Decisions

- **Reuse over new ingestion (portfolio rule 8):** no new dataset. Both deployments serve an
...
- **Least-privilege IAM instead of the bootcamp's admin user.** The original exercise attached
  `AdministratorAccess` to a new IAM user — fast to teach, wrong to ship. Here the EC2 role can read
  exactly one S3 bucket and nothing else; there is no IAM user or console access created at all.
...
- **Terraform over console clicks.** The bootcamp was taught through the AWS console UI. Every
  resource here is defined in `.tf` files so the deploy is reviewable, reproducible, and fully torn
  down with `terraform destroy` — no orphaned resources left running up a bill after the screenshots
  are taken.
- **Streamlit on port 80 via systemd**, not a manual `nano` + foreground `streamlit run` session —
  the instance survives a reboot and doesn't depend on an open SSH session staying alive.
...
**What's pending — a real AWS account, no working credentials in this environment:** the actual
`terraform apply`, the S3 upload, and the live URLs. Full checklist:
...
```bash
# 1. Regenerate the data export (needs a built AE-01 — warehouse.duckdb present)
...
The AWS fundamentals this project rebuilds — account setup, IAM, S3 static hosting, an EC2 instance
serving a Streamlit app — were originally learned in a cloud bootcamp (*Jornada de Dados*, taught by
Luciano). All code in this repository is a from-scratch rewrite: least-privilege IAM instead of an
admin user, Terraform instead of console clicks, and real portfolio data instead of the tutorial
dataset.
...
- `terraform destroy` after evidence is captured — this project doesn't need to stay running.
